...
test("Loading indicator works properly", async () => {
  const fn = jest.fn();
  ...
  const loading = await getByTestId("loading-indicator");
...
test("Low to high sorting works properly", async () => {
  const { debug, getByTestId, getAllByTestId, findAllByTestId } = render(
    <App />
  );
  ...
  expect(sortButton.textContent.trim()).toBe("Top 10 Employees");
  await sleep(3000);
  ...
  employeesData.map((el, index) => {
    expect(el.name.textContent.trim()).toBe(lowToHighData[index].name);
  });
...
test("Data fetching is correct", async () => {
  ...
  await sleep(8000);
  let data = [];
  d.forEach((a) => data.push(...a.data));
  data = data.sort((a, b) => b.salary - a.salary).slice(0, 10);
...
test("Data fetching is correct 2", async () => {
  fetchMock(
  ...
  [(1, 2, 3, 4)].forEach((p) => {
    fetchMock(
      `https://dbioz2ek0e.execute-api.ap-south-1.amazonaws.com/mockapi/get-products?page=${p}&limit=10`,
      d2[p - 1]
    );
  });
  const { getByTestId, findAllByTestId } = render(<App data={[...d2]} />);
  await sleep(4000);
  let data = [];
  d2.forEach((a) => data.push(...a.data));
  data = data.sort((a, b) => b.price - a.price).slice(0, 10);
